...
- Workspace mount: host `WORKSPACE_DIR` -> container `/workspace`
...
- optionally `WORKSPACE_DIR` to an absolute host path
...
`WORKSPACE_DIR` is mounted into `/workspace` inside the `opencode` container.
